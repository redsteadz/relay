...
```bash
uvx --from "graphifyy[gemini,sql]==0.9.49" graphify extract . --backend gemini --model gemini-3.5-flash-lite --mode deep --force --no-cluster --token-budget 5000 --max-concurrency 1
uvx --from "graphifyy[gemini,sql]==0.9.49" graphify extract . --backend gemini --model gemini-3.5-flash-lite --mode deep --no-cluster --token-budget 2000 --max-concurrency 1
